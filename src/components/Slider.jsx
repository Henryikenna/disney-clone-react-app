...
function Slider() {
  ...
  const getTrendingMovies = () => {
    GlobalApi.getTrendingVideos.then((resp) => {
      console.log("trending videos", resp.data.results);
  ...
  return(
    <div>
        ...
        {moviesList.map((item)=>(
            <img className="min-w-full md:h-[310px] object-cover object-left-top mr-5 rounded-md hover:border-[4px] border-gray-400 cursor-pointer transition-all duration-100 ease-in" src={IMAGE_BASE_URL+item.backdrop_path} alt="" />
        ))}
    </div>
    </div>
    );
